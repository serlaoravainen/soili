...
const ScheduleTable: React.FC<ScheduleTableProps> = () => {
  ...
  const filteredEmployees = useMemo(() => {
    ...
    return employees.filter((emp) => {
      ...
      if (term.length > 0) {
        const hay =
          normalizeFi(emp.name) + " " + normalizeFi(emp.email) + " " + normalizeFi(emp.department);
  ...
  const [absencesMap, setAbsencesMap] = useState<
    Record<string, { type: "absent" | "holiday"; reason: string }>
  >({});
    ...
    switch (shift.type) {
      ...
      default:
        return {
          ...
          icon: <Plus className="w-3 h-3 opacity-0 group-hover:opacity-50" />,
        };
    }
  };
            ...
            <div className="text-sm text-muted-foreground bg-accent/40 px-3 py-2 rounded-md inline-flex items-center gap-3 flex-wrap">
              ...
              <div className="flex flex-wrap gap-x-3 gap-y-1">
                {stateFilterActive && (
                  <span>
                    {filters.showActive
                      ? "Vain aktiiviset työntekijät"
                      : "Vain ei-aktiiviset työntekijät"}
                  </span>
                )}
                {filters.departments.length > 0 && (
            ...
            <div className="min-w-full">
              ...
              <div className="bg-muted/50 border-b">
                <div
                  ...
                  style={{
                    gridTemplateColumns: `minmax(200px,280px) repeat(${days}, minmax(96px, 1fr))`,
                  }}
                >
