...
type State = {
  ...
  // Toiminnot
  hydrate: (payload: { employees: Employee[]; dates: DateCell[]; shifts: ShiftRow[] }) => void;
  ...
  persist(
    devtools((set, get) => ({
      ...
      undo: () => {
        ...
        const nextPending = {
          ...pending,
          [k]: { employee_id: last.employee_id, work_date: last.work_date, hours: last.hours ?? 0 },
        };
      ...
      redo: () => {
        ...
        set({
          ...
          undoStack: [
            ...undoStack,
            { employee_id: next.employee_id, work_date: next.work_date, hours: prev?.hours ?? 0 },
          ],
          dirty: true,
        });
      },
