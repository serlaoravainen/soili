...
import { Settings, SettingsSchema, DEFAULT_SETTINGS, Theme } from "@/lib/settingsSchema";
...
type State = {
  ...
  updateExport: <K extends keyof Settings["export"]>(key: K, value: Settings["export"][K]) => void;
  ...
  updateSystem: <K extends keyof Settings["system"]>(key: K, value: Settings["system"][K]) => void;

  importFromJson: (raw: unknown) => { ok: true } | { ok: false; error: string };
        ...
        try {
          ...
          if (!persisted || typeof persisted !== "object") {
            return {
              ...
              updateNotifications: () => {},
              updateSystem: () => {},
              importFromJson: () => ({ ok: false, error: "not implemented" }),
              exportToFile: () => {},
            } as State;
          }
          ...
          const nextNotifs: Settings["notifications"] = {
            ...prevNotifs,
            adminNotificationEmails: Array.isArray(prevNotifs?.adminNotificationEmails)
            ...
            dailyDigest: prevNotifs?.dailyDigest ?? DEFAULT_SETTINGS.notifications.dailyDigest,
            digestTime: prevNotifs?.digestTime ?? DEFAULT_SETTINGS.notifications.digestTime,
          };
