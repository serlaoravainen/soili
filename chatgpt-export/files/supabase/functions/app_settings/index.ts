...
const DEV_ALLOW_NOAUTH = (Deno.env.get("DEV_ALLOW_NOAUTH") ?? "false").toLowerCase() === "true";
...
serve(async (req) => {
  ...
  if (!devBypassOk) {
    ...
    if (!ok) {
      return new Response(JSON.stringify({ code: 401, message: "Missing/invalid authorization" }), {
        status: 401,
  ...
  try {
    if (req.method === "GET") {
      ...
      return new Response(JSON.stringify({ notifications: payload }), {
        headers: { ...base, "content-type": "application/json" },
      });
    }
    ...
    if (req.method === "PUT") {
      ...
      if (!parsed.success) {
        ...
        return new Response(JSON.stringify({ error: `Invalid notifications payload: ${msg}` }), {
          status: 400,
          headers: { ...base, "content-type": "application/json" },
        });
      }
